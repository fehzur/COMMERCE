...
$img=$bdd->prepare('SELECT images FROM users WHERE id=?');
$img->execute(array($getid));
$sary=$img->fetch()['images'];
while($message=$recupMessage->fetch()){
?>
<img src="<?= $sary; ?>" width="9.8%">
<h1>Bonjour Monsieur le PDG,</h1><br>
<p style="text-decoration:underline;text-align: left; margin-left: 40px;" id="texte">Demande de congé numéro <?= $message['numero']; ?></p><br>
<p>Aujourd'hui le <?= $message['time']; ?>, à <?= $message['hours']; ?> :</p>
<p>Je voudrais vous demandez la permission de m'absenter pour prendre un congé Monsieur.</p>
<p>Je vais m'absenter le <?= $message['date_debut']; ?> jusqu' en <?= $message['date_fin']; ?> à cause d' une <?= $message['etat']; ?></p><br>
<p>En faite, <?= $message['messages']; ?></p>. Alors je dois m'absenter pour ce jour.<br>
<p>J' éspere que vous me comprennez alors j'éspere avoir une bonne réponse de votre part. Merci</p>
<div class="choix">
<form action="decision.php?id=<?= $getid; ?>" method="POST">
    <input type="submit" name="ACCORDER"value="ACCORDER" style="background: green;">
</form>
<form action="decision.php?id=<?= $getid; ?>" method="POST">
    <input type="submit" name="REFUSER" value="REFUSER" style="background: red;">
</form>
</div>
<p>------------------------------------------------------------------------------------------------------------------------------------</p>
<?php
